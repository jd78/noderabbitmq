...
let connect = () =>
    new Promise((resolve, reject) => {
        ...
        amqp.connect(configuration.serverUrl).then(conn => {
            conn.on('error', (err) => {
                _isConnected = false
                if(err.message.indexOf("exclusive use") == -1) {
                    configuration.errorLog(err)
                    throw new Error(err)
                } else {
                    _logOnce.execute(() => configuration.infoLog('consumer is passive'))
                }
            })
            
            conn.on('blocked', reason => {
                configuration.errorLog(reason)
            })

            conn.on('unblocked', () => {
                configuration.infoLog('connection unblocked')
            })
...
let startConsumer = queue => {
    ...
    _connection.createChannel().then(ch => {
        ...
        ch.consume(
            ...
            { noAck: !configuration.consumerAck, exclusive: configuration.exclusiveConsumer }
        ).then(() => {
            if(configuration.exclusiveConsumer) {
                configuration.errorLog(util.format("consumer is active on queue %s",  queue))
            }
        }).catch(err => {
            if(err.message.indexOf("exclusive use") != -1) {
                setTimeout(() => connect().then(() => startConsumer(queue)), configuration.passiveConsumerRertyConnectInMilliseconds)
                return
            }
            reject(err)
        })
