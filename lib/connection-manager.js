...
let assertExchange = (ch, name, type, durable) => new Promise((resolve, reject) => {
    ch.assertExchange(name, type, { durable: durable }).then(() => resolve()).catch(err => reject(err))
})

let assertQueue = (ch, name, durable, autodelete, options) => new Promise((resolve, reject) => {
    ch.assertQueue(name, { durable: durable, autoDelete: autodelete, arguments: options }).then(() => resolve()).catch(err => reject(err))
})
